...
class BacktrackingSearch():
    ...
    def solve(self, csp, mcv = False, ac3 = False):
        ...
        self.print_stats()
        players = []
        for var in self.allAssignments[0]:
            val = self.allAssignments[0][var]
            if isinstance(val, str):
                players.append((var[0], val))
    ...
    def backtrack(self, assignment, numAssigned, weight):
        ...
        if numAssigned == self.csp.numVars:
            ...
            if len(self.optimalAssignment) == 0 or weight >= self.optimalWeight:
                ...
                print "\n Found assignment: ", weight, self.optimalWeight, newAssignment
            return
        ...
        while len(toUpdate) > 0:
            ...
            for var2 in self.csp.get_neighbor_vars(var1):
                ...
                for val2 in self.domains[var2]:
                    for val1 in domain1:
                        if not (self.csp.binaryFactors[var1] and self.csp.binaryFactors[var1][var2] and self.csp.binaryFactors[var1][var2][val1][val2] == 0):
                            domain2.append(val2)
                            break
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    # if len(variables) == 0:
    #     sum_domain = [i for i in range(maxSum+1)]
    #     sum_name = 'sum' + str(name)
    #     csp.add_variable(sum_name, sum_domain)
    #     csp.add_unary_factor(sum_name, lambda x: x <= maxSum and x == 0)
    #     return sum_name
    minval = 4
    maxval = 10
    ...
    for n, var in enumerate(variables):
        print "sum_progress: ", var, "\n"
        newvar = 'sum' + str(name) + str(var)        
        if prevvar == None:
            domain1 = [(0, j) for j in range(minval, maxval+1)]
            csp.add_variable(newvar, domain1)
            print "added variable: ", newvar, domain1
        else:
            localmax = min(n*maxval,maxSum) + 1
            localmax2 = min((n+1)*maxval,maxSum) + 1
            domain = [(i, j) for i in range(n*minval, localmax) for j in range(i, localmax2)]
            csp.add_variable(newvar, domain)
            print "added variable: ", newvar, domain
            csp.add_binary_factor(prevvar, newvar, lambda x, y: x[1] == y[0])
        csp.add_binary_factor(var, newvar, lambda x, y: y[1] == y[0] + x)
        prevvar = newvar
    sum_domain = [i for i in range(maxSum+1)]
    sum_name = 'sum' + str(name)
    csp.add_variable(sum_name, sum_domain)
    csp.add_binary_factor(sum_name, prevvar, lambda x, y: x == y[1])
    csp.add_unary_factor(sum_name, lambda x: x <= maxSum)
